...
function Reports() {
  ...
  const download = async () => {
    ...
    try {
      ...
      // PDF-safe standard font for every string, incl. amounts
      ...
      doc.setFontSize(11);
      doc.text(`Monthly Report - ${label}`, 14, 24);
      ...
      doc.text(`Total Income: ${fmtPdfINR(q.data.totalInc)}`, 14, y);
      doc.text(`Total Expense: ${fmtPdfINR(q.data.totalExp)}`, 110, y);
      y += 7;
      doc.setFontSize(13);
      ...
      // A4 = 210mm wide, 14mm margins -> 182mm usable
      const usable = 182;
      ...
      if (q.data.byCat.length) {
        autoTable(doc, {
          ...
          tableWidth: usable,
          head: [["Category", "Total"]],
          body: q.data.byCat.map(([c, v]) => [c, fmtPdfINR(v)]),
          theme: "grid",
          headStyles: { fillColor: green, textColor: 255, font: "helvetica" },
          styles: { fontSize: 10, font: "helvetica", overflow: "linebreak", cellPadding: 2 },
          columnStyles: {
            0: { cellWidth: usable * 0.6 },
            1: { cellWidth: usable * 0.4, halign: "right", overflow: "visible" },
          },
        });
      ...
      if (q.data.rows.length) {
        autoTable(doc, {
          ...
          theme: "striped",
          headStyles: { fillColor: green, textColor: 255, font: "helvetica" },
          styles: { fontSize: 9, font: "helvetica", overflow: "linebreak", cellPadding: 2 },
          columnStyles: {
            0: { cellWidth: usable * 0.2 },
            1: { cellWidth: usable * 0.18 },
            2: { cellWidth: usable * 0.42 },
            3: { cellWidth: usable * 0.2, halign: "right", overflow: "visible" },
          },
        });
      }
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`Generated by Ghar Kharcha Manager - Page ${i} / ${pageCount}`, 14, 290);
      }
